...
import { fetchSupervisorById, updateSupervisor } from '@/shared/lib/supervisors';
...
const EditSupervisor = () => {
    ...
    useEffect(() => {
        const loadSupervisor = async () => {
            ...
            try {
                const data = await fetchSupervisorById(id);
                setFormData({
                    name: data?.name || '',
                    email: data?.email || '',
                ...
                await Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error?.response?.data?.message || error?.message || 'Failed to fetch supervisor details.',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.name.trim()) {
            await Swal.fire({
                icon: 'warning',
                title: 'Name Required',
                text: 'Please enter the supervisor name.',
                confirmButtonText: 'OK'
            });
            return;
        }
        ...
        if (!formData.email.trim()) {
            await Swal.fire({
                ...
                text: 'Please enter the supervisor email.',
                confirmButtonText: 'OK'
            });
            return;
        }
        ...
        try {
            ...
            await updateSupervisor(payload);
            
            await Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: 'Supervisor has been updated successfully.',
                confirmButtonText: 'OK'
            });

            // Redirect to supervisors list
            router.push('/supervisors');
        } catch (error: any) {
            ...
            await Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error?.response?.data?.message || error?.message || 'Failed to update supervisor. Please try again.',
                confirmButtonText: 'OK'
            });
        } finally {
            setLoading(false);
    ...
    if (fetching) {
        return (
            <Fragment>
                <Seo title="Edit Supervisor" />
                <Pageheader currentpage="Edit Supervisor" activepage="Supervisors" mainpage="Edit Supervisor" />
                        ...
                        <div className="xl:col-span-8 lg:col-span-8 md:col-span-12 col-span-12 mx-auto">
                            <div className="box">
                                <div className="box-body text-center py-8">
                                    ...
                                    <p className="mt-2 text-gray-500">Loading supervisor details...</p>
                                </div>
                            </div>
                        </div>
                    </div>
