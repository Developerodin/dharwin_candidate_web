...
const mergeNavigationStructures = (
  ...
  const merged: NavigationPermissions = JSON.parse(
...
const EditRolePage = () => {
  ...
  const subRoleId = searchParams.get("id");

  const [loading, setLoading] = useState(false);
  const [loadingSubRole, setLoadingSubRole] = useState(true);
  ...
  useEffect(() => {
    const loadSubRole = async () => {
      if (!subRoleId) {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Sub-role ID is required",
      ...
      setLoadingSubRole(true);
      setError(null);

      try {
        const subRoleData = await fetchSubRoleById(subRoleId);
        ...
        setFormData({
          ...
          isActive: subRoleData.isActive !== undefined ? subRoleData.isActive : true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateSubRole(subRoleId, payload);

      Swal.fire({
        icon: "success",
        title: "Success!",
        text: "Sub-role updated successfully",
        confirmButtonText: "OK",
      }).then(() => {
        router.push("/rbac/roles");
      });
  ...
  if (loadingSubRole) {
    return (
      <Fragment>
        <Seo title="Edit Sub-Role" />
        <Pageheader
          currentpage="Edit Sub-Role"
          activepage="Settings"
          mainpage="Roles"
        />
            ...
            <div className="xl:col-span-10 lg:col-span-10 md:col-span-12 col-span-12 mx-auto">
              <div className="box">
                <div className="box-body text-center py-12">
                  ...
                  <p className="text-gray-500 text-sm">Loading sub-role data...</p>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div className="space-y-6">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span
                              className={`text-sm font-medium ${
                                formData.isActive ? "text-success" : "text-danger"
                              }`}
                            >
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={handleInputChange}
                        rows={3}
                        placeholder="Optional description for this sub-role"
                      />
                    </div>
