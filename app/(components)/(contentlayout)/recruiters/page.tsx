...
import { fetchAllRecruiters, deleteRecruiter, fetchRecruiterById } from '@/shared/lib/recruiters';
...
const Recruiters = () => {
    ...
    const getRecruiters = useCallback(async () => {
        ...
        try {
            ...
            const data = await fetchAllRecruiters(params);
            
            // Handle API response structure
            if (data && data.results) {
                setRecruiterData(data.results);
    ...
    // Fetch recruiters when filters change
    useEffect(() => {
        getRecruiters();
    }, [getRecruiters]);
    ...
    // Function to open recruiter details modal
    const openRecruiterModal = async (recruiter: any) => {
        try {
            const fullData = await fetchRecruiterById(String(recruiter?.id ?? recruiter?._id));
            setSelectedRecruiter(fullData);
            setShowModal(true);
        } catch (err: any) {
            await Swal.fire({
                icon: 'error',
                title: 'Error',
                text: err?.message || 'Failed to fetch recruiter details',
                    ...
                    <div className="box">
                        <div className="box-header justify-between flex-wrap">
                            <div className="box-title">
                                Recruiter List {totalResults > 0 && <span className="text-sm font-normal text-gray-500">({totalResults} recruiters)</span>}
                            </div>
                            <div className="flex flex-wrap gap-2">
                                ...
                                <button type="button" className="ti-btn ti-btn-primary !bg-primary !text-white !py-1 !px-2 !text-[0.75rem] !m-0 !gap-0 !font-medium">
                                    <i className="ri-add-line font-semibold align-middle"></i> <Link href="/recruiters/add">Add Recruiter</Link>
                                </button>
                            </div>
                        </div>
                        <div className="box-body">
                            {loading ? (
                                <div className="text-center py-8">
                                    ...
                                    <p className="mt-2 text-gray-500">Loading recruiters...</p>
                                </div>
                            ) : error ? (
                                <div className="text-center py-8">
                                        ...
                                        <tbody>
                                            {recruiterData.map((recruiter, i) => (
                                                <tr 
                                                    ...
                                                    key={recruiter?.id || recruiter?._id || Math.random()}
                                                ...
                                                >
                                                        ...
                                                        <div className="flex items-center leading-none">
                                                            <div className="me-2">
                                                                <span className="avatar avatar-md avatar-rounded">
                                                                    <img src={recruiter?.avatar || "/assets/images/faces/1.jpg"} alt="img" />
                                                                </span>
                                                            </div>
                                                            <div>
                                                                <span className="block font-semibold mb-1">{recruiter?.name}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <span className="text-[#8c9097] dark:text-white/50 text-[0.75rem]">{recruiter?.email}</span>
                                                    </td>
                                                    <td>
                                                        <span className={`badge ${recruiter?.isEmailVerified ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                                                    ...
                                                    <td>
                                                        <div className="flex flex-row items-center !gap-2 text-[0.9375rem]" onClick={(e) => e.stopPropagation()}>
                                                            ...
                                                            <Link 
                                                                aria-label="anchor" 
                                                                href={`/recruiters/edit?id=${encodeURIComponent(String(recruiter?.id ?? recruiter?._id))}`} 
                                                                scroll={false} 
                                                                    ...
                                                                    onClick={(e) => {
                                                                        ...
                                                                        Swal.fire({
                                                                            ...
                                                                            if (result.isConfirmed) {
                                                                                try {
                                                                                    await deleteRecruiter(String(recruiter?.id ?? recruiter?._id));
                                                                                    // Refresh recruiters list after deletion
                                                                                    getRecruiters();
                                                                                    await Swal.fire(
                                                                                        "Deleted!",
                                                                                        "The recruiter has been deleted.",
                                                                                        "success"
                                                                                    );
                                                                                } catch (e: any) {
                                                                                    await Swal.fire(
                                                                                        "Delete failed",
                                                                                        e?.response?.data?.message || e?.message || "Unable to delete recruiter.",
                                                                                        "error"
                                                                                    );
                                                                                }
                                                                            }
                        ...
                        <div className="inline-block align-bottom bg-white dark:bg-bodybg rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                            <div className="bg-white dark:bg-bodybg px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Recruiter Details</h3>
                                    <button
                                        onClick={closeModal}
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-gray-900 dark:text-white">{selectedRecruiter?.name || '-'}</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                                        <p className="mt-1 text-sm text-gray-900 dark:text-white">{selectedRecruiter?.email || '-'}</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Role</label>
                                        <p className="mt-1 text-sm text-gray-900 dark:text-white">{selectedRecruiter?.role || '-'}</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email Verified</label>
                                        <p className="mt-1">
                                            <span className={`badge ${selectedRecruiter?.isEmailVerified ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                                                {selectedRecruiter?.isEmailVerified ? 'Verified' : 'Unverified'}
                                            </span>
                                        </p>
                                    </div>
                                    {selectedRecruiter?.createdAt && (
                                        <div>
                                            ...
                                            <p className="mt-1 text-sm text-gray-900 dark:text-white">
                                                {new Date(selectedRecruiter.createdAt).toLocaleString()}
                                            </p>
                                        </div>
                                    )}
                                    {selectedRecruiter?.updatedAt && (
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Updated At</label>
                                            <p className="mt-1 text-sm text-gray-900 dark:text-white">
                                                {new Date(selectedRecruiter.updatedAt).toLocaleString()}
                                            </p>
                                        </div>
                                    )}
                                </div>
